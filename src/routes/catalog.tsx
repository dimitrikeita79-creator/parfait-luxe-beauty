        ...
        {CATALOG.map((c, i) => {
          const inner = (
            <Frame tone={c.tone} rounded="rounded-[28px]" className="aspect-[4/5] w-full">
              {c.comingSoon && (
                <span className="absolute right-2 top-2 z-10 rounded-full bg-black/60 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-white">
                  Bientôt
                </span>
              )}
              <div className="absolute inset-x-0 bottom-0 p-4">
                <p className="font-display text-xl font-semibold text-white drop-shadow-lg">{c.name}</p>
                <p className="mt-0.5 text-[11px] font-medium text-white/90">{c.countLabel}</p>
              </div>
            </Frame>
          );
          return c.comingSoon ? (
